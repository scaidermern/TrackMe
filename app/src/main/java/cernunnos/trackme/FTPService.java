...
public class FTPService extends IntentService {
    ...
    protected void doFTPStuff(final ArrayDeque<MyLocation> locations) {
        ...
        try {
            ftp.changeWorkingDirectory("/httpdocs/scy/trackme");
        } catch (IOException e) {
            Log.e(TAG, "onLocationChanged(): could not change working directory: " + e.getMessage());
            if (ftp.isConnected()) {
                try {
                    ftp.disconnect();
                } catch (IOException f) {
        ...
        try {
            OutputStream file = ftp.storeFileStream("locations.txt");
            if (file == null) {
                Log.e(TAG, "onLocationChanged(): could not open file: code: " + ftp.getReplyCode());
                if (ftp.isConnected()) {
                    try {
                        ftp.disconnect();
                    } catch (IOException f) {
